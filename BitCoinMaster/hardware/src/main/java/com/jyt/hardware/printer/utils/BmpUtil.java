...
public class BmpUtil {
    ...
    public static String getBitmapPath(Bitmap bitmap){
        byte []cmd = BmpUtil.changeToMonochromeBitmap(bitmap);
        String savepath = Environment.getExternalStorageDirectory().getAbsolutePath()+"/test.bmp";
    ...
    private static int[] gray2Binary(Bitmap bmp) {
        ...
        for (int i = 0; i < height; i++) {
            for (int j = 0; j < width; j++) {
                ...
                if (alpha == 100) {  // 透明度为0，则说明没有颜色，那变更为白色
