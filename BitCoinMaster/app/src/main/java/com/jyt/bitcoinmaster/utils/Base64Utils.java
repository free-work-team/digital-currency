...
public class Base64Utils {
	 ...
	 * @throws IOException
	 */
	public FileOutputStream base64ToFile(String strBase64, File outFile) {
		try {
			// 解码，然后将字节转换为文件
			byte[] bytes = Base64.decode(strBase64); // 将字符串转换为byte数组
			FileOutputStream out =  copyByte2File(bytes, outFile);
			return  out;
		} catch (Exception ioe) {
			ioe.printStackTrace();
			return null;
	 ...
	 * @param file
	 */
	private FileOutputStream copyByte2File(byte[] bytes, File file) {
	...
	public static void testJpg2Base64() {
	...
	public static String base642File(String str) {
		Base64Utils utils = Base64Utils.getInstance();
		String videoPath = Environment.getExternalStorageDirectory().getPath()+ File.separator + "HungHui" + File.separator ;
